function GetPlayerInventory(xPlayer, showHotbar)
    ...
    local simCards = exports['213-Main']:GetPlayerSimCards(xPlayer.source)
    for k,v in pairs(simCards) do
        table.insert(items, {
            type = 'item_sim',
            name = 'sim',
            icon = 'sim',
    ...
    for plate, value in pairs(exports['213-Main']:GetPlayerCarKeys(xPlayer.source)) do
        table.insert(items, {
            type = 'item_carkey',
            name = 'carkey',
            icon = 'karta_dostepu',
            label = "Kluczyki #" .. plate,
            plate = plate,
            count = 1,
            use = false,
            remove = true
    ...
    -- local carkeys = exports['213-Main']:GetPlayerCarKeys(xPlayer.source)
    ...
    if showHotbar ~= false then
            ...
            if not Inventories['hotbar'][identifier] then
                ...
                if Config.HotbarSave then
                    MySQL.Async.fetchScalar("SELECT hotbar FROM users WHERE identifier=@id", {
                        ["@id"] = identifier
                    }, function(result)
                        if result then
                            Inventories['hotbar'][identifier] = json.decode(result) or {}
                            done:resolve()
                        end
                    end) 
                else 
...
function GetInventory(xPlayer, inventory)
    ...
    for k,v in pairs(Inventories[inventory.type][inventory.id]) do
        ...
        elseif v.type == 'item_carkey' then
            table.insert(items, {
                type = v.type,
                name = v.name,
                label = 'Klucze #' .. v.plate,
                count = v.count,
                plate = v.plate
            }) 
...
function AddItemToInventory(xPlayer, item, count, inv, cb)
    ...
    if inv.weight then
        ...
        if newWeight > inv.weight then 
            xPlayer.triggerEvent('inventory:notify', 'error', 'Ekwipunek jest za cięzki!')
            return
        end 
    end
    ...
    if not found then
        table.insert(inventory, {type = item.type, name = item.name, count = count, plate = item.plate or nil})
    end
    ...
    for k,v in pairs(inventory) do
        if v.name == item.name then
            ...
            if item.type == 'item_weapon' then
                ...
                cb()
            elseif item.type == 'item_carkeys' then
                table.remove(inventory, item.index+1) 
                cb()
            else 
