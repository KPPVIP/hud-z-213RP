--[[if Config.Rob then
    ...
    RegisterCommand('rob', function()
        ...
        if player ~= -1 and distance <= 3.0 then
           ...
           TriggerEvent('inventory:notify', 'error', 'Nie ma w pobliżu gracza!') 
        end
    end, false)
...
AddEventHandler('213-Kajdanki:OtworzJebaneMenuPodpierdalaniaItemow', function(player)
    OpenInventory({
        id = GetPlayerServerId(player),
        type = 'player',
        title = GetPlayerServerId(player),
        weight = Config.PlayerWeight,
        timeout = Config.RobTimeout
    })
end)