...
RegisterNUICallback('MoveItemToPlayer', function(data, cb)
    if data.player then
        TriggerServerEvent('inventory:giveItemToPlayer', data.item, tonumber(data.count), {id = data.player})
        SendNUIMessage({
            action = 'close',
...
RegisterNUICallback('UseItem', function(data)
    ...
    if data.item.type == 'item_sim' then
        TriggerServerEvent('213-SimCards:UseSimCard', data.item.number)
    else
...
RegisterNUICallback('RemoveItem', function(data, cb)
    ...
    for k,v in pairs(Drops) do
        ...
        if distance < 2.0 then
            TriggerServerEvent('inventory:moveItemToOther', data.item, tonumber(data.count), {id = k, type = 'drop', title = '🗑️ Podłoga - ' .. k})
            Wait(50)
            cb(200)
            return
        end
...
RegisterNUICallback('close:zdrapka', function()
...
RegisterCommand('openInventory', function()
    ...
    if IsPedInAnyVehicle(PlayerPedId(), true) then
        ...
        for k,v in pairs(Drops) do
            local distance = #(v.coords - GetEntityCoords(PlayerPedId()))
            if distance < 2.0 then
                OpenInventory({id = k, type = 'drop', title = '🗑️ Podłoga - ' .. k, coords = v.coords, weight = false, timeout = 1000})
                return
            end
        end 
        OpenInventory()
