...
AddEventHandler('213_blackout:pasy', function(status)
	beltOn = status
end)

AddEventHandler('213_dzwon:display', function(status)
	beltStatus = status
end)
					...
					if IsCar(oldVehicle, false) then
						...
						if not isBlackedOut then
							...
							if (currentDamage < oldDamage and (oldDamage - currentDamage) >= 150) or (fall and speed > (vehicleClass == 8 and 5 or 30)) then
								...
								dzwonCalled = true
								TriggerServerEvent('213_blackout:dzwon', list, damage)
							end
						end
...
RegisterNetEvent('213_blackout:impact')
AddEventHandler('213_blackout:impact', function(speedBuffer, velocityBuffer)
	CreateThread(function()
...
AddEventHandler('213_blackout:belt', function(status)
	local ped = PlayerPedId()
	if IsPedInAnyVehicle(ped, false) then
		beltOn = status
		TriggerEvent('HUD:Update', {
		...
		if IsPedInAnyVehicle(ped, false) then
			...
			if vehicle ~= 0 and IsCar(vehicle, true) then
				if IsControlJustReleased(0, 29) then
					TriggerEvent('213_blackout:belt', not beltOn)
				end
				...
				if GetPedInVehicleSeat(vehicle, -1) == ped then
					...
					if speedBuffer[2] ~= nil and not beltCalled and speedBuffer[2] > 27.77 and (speedBuffer[2] - speedBuffer[1]) > (speedBuffer[1] * 0.25) and not GetPlayerInvincible(PlayerId()) and GetEntitySpeedVector(vehicle, true).y > 1.0 then
						...
						TriggerServerEvent('213_blackout:dzwon', list, 10)
						TriggerServerEvent('213_blackout:impact', list, speedBuffer, velocityBuffer)
					end
